...
public class UserServiceImpl implements UserService {
	...
	public Optional<UserEntity> findByUserId(String userId) throws BusinessResourceException {
		...
		if (Boolean.FALSE.equals(userFound.isPresent())) {
			throw new BusinessResourceException("UserNotFound",
					"Aucun utilisateur n'existe avec l'identifiant : " + userId, HttpStatus.NOT_FOUND);
		}
		return userFound;
	}
	...
	public Optional<UserEntity> findByEmail(String email) throws BusinessResourceException {
		...
		if (Boolean.FALSE.equals(userFound.isPresent())) {
			throw new BusinessResourceException("User Not Found", "Aucun utilisateur avec l'email :   " + email,
	...
	public void deleteUser(String userId) throws BusinessResourceException {
		...
		try {
			...
			log.info("exist user " + user);
			userRepository.deleteById(user.getId());
		} catch (EmptyResultDataAccessException ex) {
			log.error(String.format("Aucun utilisateur n'existe avec l'identifiant: " + userId, ex));
			throw new BusinessResourceException("DeleteUserError",
					"Erreur de suppression de l'utilisateur avec l'identifiant : " + userId, HttpStatus.NOT_FOUND);
		} catch (Exception ex) {
			throw new BusinessResourceException("DeleteUserError",
					"Erreur de suppression de l'utilisateur avec l'identifiant : " + userId,
	...
	public Optional<UserEntity> findByUserNameAndPassword(String userName, String password)
		...
		try {
			...
			if (!userFromDb.isPresent())
				throw new BusinessResourceException("UserNotFound",
						"Aucun utilisateur n'existe avec le nom " + userName, HttpStatus.NOT_FOUND);
			...
			throw new BusinessResourceException("TechnicalError", "Une erreur technique est survenue",
					HttpStatus.INTERNAL_SERVER_ERROR);
		}
		return userFromDb;
	}
	...
	private Set<RoleEntity> extractRoleFromDB(Set<RoleEntity> roles, Stream<RoleEntity> rolesDb) {
		...
		return rolesDb.filter(roleDb -> roles.stream().anyMatch(role -> role.equals(roleDb)))
				.collect(Collectors.toCollection(HashSet::new));
	}

	private void updateUserRole(UserEntity user) {

		Set<RoleEntity> roles = extractRoleFromDB(
				user.getRoles().stream().collect(Collectors.toCollection(HashSet::new)),
